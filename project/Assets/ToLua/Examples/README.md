...
``` csharp
    ...
    void Start () 
    {
        ...
        if (func != null)
        {
            ...
            Debug.Log("generic call return: {0}", num);

            num = CallFunc();
            Debug.Log("expansion call return: {0}", num);

            Func<int, int> Func = luaFunc.ToDelegate<Func<int, int>>();
            num = Func(123456);
            Debug.Log("Delegate call return: {0}", num);
            
            num = lua.Invoke<int, int>("test.luaFunc", 123456, true);
            Debug.Log("luastate call return: {0}", num);
        }
...
``` csharp
    ...
    void Start () 
    {
        ...
        //通过LuaState访问
        Debug.Log("Read var from lua: {0}", lua["var2read"]);
        Debug.Log("Read table var from lua: {0}", lua["varTable.default"]);  //LuaState 拆串式table
        ...
        Debug.Log("Read varTable from lua, default: {0} name: {1}", table["default"], table["map.name"]);
        table["map.name"] = "new";  //table 字符串只能是key
        Debug.Log("Modify varTable name: {0}", table["map.name"]);

        table.AddTable("newmap");
        LuaTable table1 = (LuaTable)table["newmap"];
        table1["name"] = "table1";
        Debug.Log("varTable.newmap name: {0}", table1["name"]);
        table1.Dispose();
        ...
        if (table1 != null)
        {
            Debug.Log("varTable metatable name: {0}", table1["name"]);
        }
        ...
        {
            Debug.Log("varTable[{0}], is {1}", i, list[i]);
        }
